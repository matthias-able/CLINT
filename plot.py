...
label = True
for ion in ions:    
    kette0 = np.sum(RESP_charges[ions.index(ion)][:19]) #kette
    ring = np.sum(RESP_charges[ions.index(ion)][19:27]) #ring
    kette1 = np.sum(RESP_charges[ions.index(ion)][27:]) #kette
    if label:
        ax.plot(ion, kette0, 'x', color = 'blue', label = 'Kette0')
        ax.plot(ion, ring, 'x', color = 'red', label = 'Ring')
        ax.plot(ion, kette1, 'x', color = 'purple', label = 'Kette1')
        label = False
    ax.plot(ion, kette0, 'x', color = 'blue')
    ax.plot(ion, ring, 'x', color = 'red')
    ax.plot(ion, kette1, 'x', color = 'purple')
